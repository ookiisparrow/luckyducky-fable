...
> 实战来源：M-adminImpl（30 页照设计稿并行重写·6 批）与 admin2 迭代I（两维 sweep·34 逻辑 bug·round6 零确认闭环），账在 `docs/重构日志.md`。核心信条：**并行产出、串行亲核——验证不是相信**（元模式 A7）。用户已拍板分层执行：fable 做大脑、执行派下层模型，agent 只返回精简结构化结果。
