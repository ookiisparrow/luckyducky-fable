...
- `.claude/settings.json`：PreToolUse = guard-deploy（部署闸）；PostToolUse = check-structure（--hook）；pre-commit = npm run check。
- pre-commit（`scripts/git-hooks/pre-commit`）：conventions + lint + test。
- npm 守卫：interface-catalog-sync / writes-need-gate / flow-seam-single / visual-check RPC 探活。
...
- 判断型规则 → 成文进 `CLAUDE.md` / 加一条 `check-structure` 机器守卫 / 记 memory。
