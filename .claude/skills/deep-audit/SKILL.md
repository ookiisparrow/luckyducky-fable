...
4. **每条根因有没有机器守卫**：对 `docs/根因账本.md` 12 类逐条问"绝迹证明是机器守卫，还是只靠测试/自觉"——只靠自觉的就是缺口（曾抓到 #3 写库没过闸、#4 Fen 没接钱链）。
5. **自动化元审**：哪些规矩还靠人？能结构化的补 `scripts/check-structure.mjs` 守卫；不能的（方法论/守则）成文并写清"为什么靠人"。
6. **接口/契约**：`docs/接口正册.md` 与代码同步（`interface-catalog-sync`）；新接口登记、敏感函数入名单。
