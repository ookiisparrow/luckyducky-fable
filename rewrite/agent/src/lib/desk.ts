...
const keyOf = (m: Msg) => `${m.at}|${m.direction}|${m.text}`
...
export function normalizeMsgs(raw: unknown): Msg[] {
  ...
  for (const m of raw as Record<string, any>[]) {
    ...
    out.push({ direction: m.direction === 'out' ? 'out' : 'in', text: String(m.text || ''), at, msgtype: String(m.msgtype || '') })
  }
  return out
}

/** 增量合并（不重气泡·升序）：同 (at,方向,文) 视为同条去重；并入后整体按 at 升序。 */
