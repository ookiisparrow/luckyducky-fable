...
// kfSendText（黄金 §四·2026-07-23 拓扑收编批：原 cs/kfSend 独立函数收编进 kit/wecom.ts）。
// SERVER_ONLY 用例删除：收编后不再是独立可调用云函数，越权发送边界随函数消失——权限改由外层
// adminApi 网关闸（口令 + scope + 接待窗口，见 app-admin6.test.ts sendAgentMessage 用例）负责。
describe('kfSendText / kfHealthProbe（黄金 §四/§十一）', () => {
  it('大白话：缺参拒；未配置拒；配齐后取 token 失败拒', async () => {
    ...
    process.env.WXKF_CORPID = 'c'
    process.env.WXKF_SECRET = 's'
    stubFetch(() => ({ errcode: 40001 })) // gettoken 报错·无 access_token
    expect(((await kfSendText({ externalUserId: 'e', openKfId: 'k', text: 'hi' })) as any).error).toBe('TOKEN_FAILED')
  })

  it('大白话：配齐且取到 token 后经 send_msg 送达；平台 errcode 非零如实透出 sent:false（不抛错）', async () => {
    process.env.WXKF_CORPID = 'c'
    process.env.WXKF_SECRET = 's'
    stubFetch((url) => {
      if (url.includes('gettoken')) return { access_token: 'T', expires_in: 7200 }
    ...
    const r: any = await kfSendText({ externalUserId: 'e', openKfId: 'k', text: 'hi' })
    expect(r.ok).toBe(true)
    expect(r.sent).toBe(true)
    expect(r.msgid).toBe('m1')

    // token 已缓存（同一 access_token 有效期内）——只需重路由 send_msg 分支
    stubFetch((url) => (url.includes('send_msg') ? { errcode: 95018 } : { access_token: 'T', expires_in: 7200 }))
    const bad: any = await kfSendText({ externalUserId: 'e', openKfId: 'k', text: 'hi' })
    expect(bad.ok).toBe(true) // 校验闸本身过了（sent 语义在 sent 字段，不在 ok）
    expect(bad.sent).toBe(false)
    expect(bad.errcode).toBe(95018)
  })
...
// kfFetchMedia（B5·顾客发图坐席可见·平台接缝单点#12·2026-07-23 拓扑收编批：原 cs/kfMedia 独立函数
// 收编进 kit/wecom.ts）。SERVER_ONLY 用例删除：理由同上 kfSendText——收编后无独立可调用边界。
describe('kf/media/get 下载 + kfFetchMedia（B5·顾客发图坐席可见·平台接缝单点#12）', () => {
  ...
  it('大白话：缺参拒；未配置拒；配齐后经直连下载并回 Buffer（收编后跨函数 JSON 序列化约束消失·不再转 base64）', async () => {
    expect(((await kfFetchMedia('')) as any).error).toBe('BAD_ARGS')
    expect(((await kfFetchMedia('m1')) as any).error).toBe('KF_NOT_CONFIGURED')
