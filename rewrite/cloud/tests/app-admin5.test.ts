...
describe('知识库与节点策展（kb=公司 FAQ·checkpoints=课程节点定义）', () => {
  ...
  it('大白话：saveCheckpoints GC 删除失败不再吞（H2·同批F F1 判例）——新节点已 upsert 成功、旧节点删不掉即 ok:false + 留痕', async () => {
    ...
    expect(ok.ok).toBe(true)
    expect(control.dump('checkpoints').map((d: any) => d._id)).toEqual(['defmeta:c4', 'def:c4:new']) // 元档随整课覆盖立/更新（批A CAS）
  })

  // —— 课程级元档 CAS 乐观并发（批A·根因#1/#2·确定性 _id defmeta:<courseId>·同 saveCourseDraft rev/baseRev 口径）——
  it('大白话：baseRev 陈旧 → DRAFT_CONFLICT，且一个节点都不动（先保存者的策展不被静默覆盖）', async () => {
    control.seed('checkpoints', [
      { _id: 'defmeta:c5', type: 'defmeta', courseId: 'c5', rev: 3, updatedAt: 1 },
      { _id: 'def:c5:n1', type: 'def', courseId: 'c5', nodeId: 'n1', title: '旧节点' },
    ])
    const r = await post('saveCheckpoints', SUPER, { courseId: 'c5', baseRev: 1, nodes: [{ nodeId: 'n2', title: '新节点', order: 0 }] })
    expect(r.ok).toBe(false)
    expect(r.error).toBe('DRAFT_CONFLICT')
    expect(r.rev).toBe(3) // 回当前 rev 供前端重载
    const ids = control.dump('checkpoints').map((d: any) => d._id)
    expect(ids).toContain('def:c5:n1') // 旧节点未被删
    expect(ids).not.toContain('def:c5:n2') // 新节点未写（拒在预检后、写库前）
    expect(control.dump('checkpoints').find((d: any) => d._id === 'defmeta:c5').rev).toBe(3) // 元档 rev 未动
  })

  it('大白话：CAS 抢占失败（读到 rev 后并发方先 bump 元档）→ DRAFT_CONFLICT（诚实拒绝优于静默互吃）', async () => {
    control.seed('checkpoints', [{ _id: 'defmeta:c6', type: 'defmeta', courseId: 'c6', rev: 1, updatedAt: 1 }])
    let bumped = false
    // 读 curRev 后、CAS where 求值前，让「并发方」先把元档 rev 抢先 bump（模拟真机毫秒级并发·mock beforeUpdate 钩子）
    control.setBeforeUpdate(async ({ coll, data }: any) => {
      if (!bumped && coll === 'checkpoints' && data && typeof data.rev === 'number') {
        bumped = true // 同步置位防嵌套 update 再入
        await getDb().collection('checkpoints').doc('defmeta:c6').update({ data: { rev: 99 } })
      }
    })
    const r = await post('saveCheckpoints', SUPER, { courseId: 'c6', baseRev: 1, nodes: [{ nodeId: 'n1', title: 't', order: 0 }] })
    control.setBeforeUpdate(null)
    expect(r.ok).toBe(false)
    expect(r.error).toBe('DRAFT_CONFLICT')
    expect(control.dump('checkpoints').some((d: any) => d._id === 'def:c6:n1')).toBe(false) // 抢占失败·未进 upsert
  })

  it('大白话：defmeta 元档不出现在 listCheckpoints.list（读侧 type:def 过滤天然排除）；正常保存立元档 rev 从 1 起', async () => {
    const save = await post('saveCheckpoints', SUPER, { courseId: 'c7', nodes: [{ nodeId: 'n1', title: '节点一', order: 0 }] })
    expect(save.ok).toBe(true)
    // 元档已立（rev 从 1 起·确定性 _id）
    const meta = control.dump('checkpoints').find((d: any) => d._id === 'defmeta:c7')
    expect(meta.type).toBe('defmeta')
    expect(meta.rev).toBe(1)
    const list = await post('listCheckpoints', SUPER, { courseId: 'c7' })
    expect(list.list.map((n: any) => n.nodeId)).toEqual(['n1']) // 只 def·defmeta 不漏进
    expect(list.list.some((n: any) => !n.nodeId)).toBe(false) // 无空 nodeId 的污染条目
    expect(list.rev).toBe(1) // 响应带元档 rev 供前端做 baseRev
  })
})
