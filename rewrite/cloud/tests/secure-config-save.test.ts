...
describe('vod 凭证档（决策§29 转码管线批0：VOD 密钥/防盗链 Key/任务流模板入库·填即生效）', () => {
  it('大白话：vod 档四字段可保存合并、kit 读取生效；哨兵密钥值绝不出现在 auditLog', async () => {
    const r = await post('saveSecureConfig', SUPER, {
      docId: 'vod',
      fields: { secretId: 'AKIDvod1', secretKey: 'SENTINEL_VOD_KEY_LEAK_9f4', playKey: 'pk-1', procedure: 'LuckyDuckyVod' },
    })
    expect(r.ok).toBe(true)
    const doc = control.dump('secureConfig').find((d: any) => d._id === 'vod')
    expect(doc.secretId).toBe('AKIDvod1')
    expect(doc.procedure).toBe('LuckyDuckyVod')
    const logs = control.dump('auditLog')
    expect(JSON.stringify(logs[logs.length - 1])).not.toContain('SENTINEL_VOD_KEY_LEAK_9f4')
  })

  it('大白话：vod 档不收非白名单字段——传 wxkf 的 corpId 应 400 NO_FIELDS', async () => {
    const r = await post('saveSecureConfig', SUPER, { docId: 'vod', fields: { corpId: 'not-a-vod-field' } })
    expect(r.status).toBe(400)
    expect(r.error).toBe('NO_FIELDS')
  })
})
