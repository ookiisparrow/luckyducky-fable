...
describe('退款审批（黄金：原子抢占/回滚条件化/进课重算封顶）', () => {
  ...
  // 批K（用户拍板方案 A·2026-07-19）：refund_required＝关单后晚到支付回调、货已被买走的终态，钱已收
  // 但发不出货，此前无任何在库退款路径（只能人工去商户平台退）。放行 overrideRefund 这一个越规通道。
  it('大白话：refund_required 死信单可越规退款（钱已收货发不出·批K 方案 A）——金额仍按分摊封顶、售后单 approved+overridden、真触发退款工作流', async () => {
    const seedFlow2 = () => control.seed('config', [{ _id: 'pay', mode: 'real', refundFlowId: 'flow-refund' }])
    seedOrder({ status: 'refund_required' } as any)
    seedFlow2()
    control.setCallFunctionResult({ result: { data: { status: 'PROCESSING', refund_id: 'rq1' } } })
    const r = await post('overrideRefund', { orderId: 'o1', lineId: 'p1__红', reason: '缺货无法履约·退款' })
    expect(r.ok).toBe(true)
    const created = control.dump('afterSales').find((a: any) => a.overridden)
    expect(created.status).toBe('approved')
    expect(created.refundAmount).toBe(178) // 该行分摊≤实付 178（钱守恒公式零改动）
    expect(control.callFunctionCalls()[0].data.data.amount.refund).toBe(17800)

    // 金额红线不因本批松动：同行再退一次 → NOTHING_LEFT（行级封顶照咬 refund_required 单）
    expect((await post('overrideRefund', { orderId: 'o1', lineId: 'p1__红', reason: '再退' })).error).toBe('NOTHING_LEFT')
  })

  it('大白话：closed（关单·钱根本没收）仍被拒——放行死信不等于放行所有终态，否则等于凭空退钱给没付钱的人', async () => {
    seedOrder({ status: 'closed' } as any)
    control.seed('config', [{ _id: 'pay', mode: 'real', refundFlowId: 'flow-refund' }])
    const r = await post('overrideRefund', { orderId: 'o1', lineId: 'p1__红', reason: '试探' })
    expect(r.error).toBe('BAD_STATUS:closed')
