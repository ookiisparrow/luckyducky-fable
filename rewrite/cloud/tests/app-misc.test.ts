...
import { COLLECTIONS, SEED_COURSES } from '@ldrw/shared'
import { generateQrcodes, makeBatchId } from '../src/functions/adminApi/actions/batches'
...
// 激活码生成（黄金 §一：数量合法·防废码·批次不撞）——2026-07-23 拓扑收编批：原独立函数 ops/genQrcodes.ts
// 收编进 adminApi/actions/batches.ts generateQrcodes，原 kit.withAdminGate 管理闸不迁（唯一调用方
// createBatch 已过 adminApi 外层网关 checkKey + ACTION_CAPS，见 app-admin2.test.ts 码批次用例覆盖）——
// ADMIN_ONLY 用例随之删除，本层只测生成逻辑本身（数量/课程校验、码唯一性、批次号不撞）。
describe('激活码生成（黄金 §一：数量合法·防废码·批次不撞）', () => {
  const db = getDb()
  beforeEach(() => control.seed('courses', [{ _id: 'c1', title: '钩织入门' }]))

  it('大白话：缺课程/课程不存在拒；数量漏传/非数字/为 0 拒不再静默按 1；码唯一且用易抄写字符表；同课同秒两批不撞号', async () => {
    expect(((await generateQrcodes(db, '', 3)) as any).error).toBe('BAD_ARGS')
    expect(((await generateQrcodes(db, 'ghost', 3)) as any).error).toContain('UNKNOWN_COURSE')
    for (const bad of [undefined, 0, 'abc']) {
      expect(((await generateQrcodes(db, 'c1', bad as any)) as any).error).toBe('BAD_ARGS')
    }
    expect(control.dump('qrcodes').length).toBe(0) // 全拒·一个码没生成
    const r = (await generateQrcodes(db, 'c1', 30)) as any
...
// seedProducts 用例整段删除：函数已退役·2026-07-23 拓扑收编批（种子源 shared/seed/products.ts 一并删除）。
describe('seed / init 管理闸（黄金 §十二：防覆盖生产数据·清单=单源全量）', () => {
  it('大白话：普通用户种数据一律拒且不写库（防覆盖生产课程）；后台放行且幂等重跑不重复建档', async () => {
    control.setOpenId('oUser')
    expect(((await seedCourses({})) as any).error).toBe('ADMIN_ONLY')
    expect(control.dump('courses').length).toBe(0)
    control.setOpenId('')
    ...
    await seedCourses({}) // 幂等：id 作 _id·set upsert·重跑不翻倍
    expect(control.dump('courses').length).toBe(SEED_COURSES.length)
  })
