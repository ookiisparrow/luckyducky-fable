...
describe('物料主档（黄金 §E：料号契约·计量锁死·调整幂等）', () => {
  ...
  it('大白话：库存调整必留原因、量须非零整数；同调整号重放不双记账；扣超余额拒且流水零残留', async () => {
    ...
    // 深审 P2：成品 fg: 调整拒（applyStockMoves 跳 fg casChange＝假成功账实分叉·成品调整走 inventory）
    expect((await post('adjustStock', SUPER, { materialId: 'fg:p1__红', adjustId: 'af', delta: 5, reason: '期初' })).error).toBe('FG_NOT_ADJUSTABLE')
    expect(control.dump('stockLedger').some((l: any) => l._id === 'adjust:af:fg:p1__红')).toBe(false) // 无 stray fg 流水
  ...
  it('大白话（D2）：纯空格原因不能绕过 NO_REASON——trim 后判空；有效原因落库为 trim 后的值', async () => {
    control.seed('materials', [mat('stuffing', 10)])
    expect((await post('adjustStock', SUPER, { materialId: 'stuffing', adjustId: 'b1', delta: 1, reason: '   ' })).error).toBe('NO_REASON')
    const r = await post('adjustStock', SUPER, { materialId: 'stuffing', adjustId: 'b2', delta: 1, reason: ' 盘点 ' })
    expect(r.applied).toBe(1)
    expect(control.dump('stockLedger').find((l: any) => l._id === 'adjust:b2:stuffing').reason).toBe('盘点') // 落库已 trim
  })
})
...
describe('外协线（黄金 §G：发料全有或全无·收货定格应付损耗·结算销账）', () => {
  ...
  it('大白话（D3）：发料行数上限 50——51 行拒 BAD_LINES、50 行放行（同 scmPurchase MAX_LINES 判例·防垃圾大单）', async () => {
    const lines50 = Array.from({ length: 50 }, (_, i) => ({ materialId: `yarn:c${i}:L:raw`, qty: 1 }))
    const lines51 = [...lines50, { materialId: 'yarn:c50:L:raw', qty: 1 }]
    control.seed('materials', lines50.map((l) => mat(l.materialId, 0))) // saveOutwork 亦查主档在册（NO_MATERIAL 闸）
    expect((await post('saveOutwork', SUPER, { workerId: 'w1', pieceRateFen: 1, issueLines: lines51 })).error).toBe('BAD_LINES')
    const ok = await post('saveOutwork', SUPER, { workerId: 'w1', pieceRateFen: 1, issueLines: lines50 })
    expect(ok.ok).toBe(true)
  })
})
...
describe('配方管理与组装（黄金 §H/§I：契约校验·快照冻结·幂等·全单回滚）', () => {
  ...
  it('大白话（战役3 批D·D1）：productId 半边同样受定界符防护——含 __ 或尾随 _ 一律拒 BAD_SPEC；合法组合（如 (\'p\',\'_q\')）过校验、只在缺差异位时才见 NO_PROFILE', async () => {
    seedBom()
    // ('p_','q')：productId 尾随 _ → BAD_SPEC（D1 撞键案例之一，早于 NO_PROFILE 校验）
    expect((await post('runAssembly', SUPER, { assemblyId: 'x1', productId: 'p_', spec: 'q', sets: 1 })).error).toBe('BAD_SPEC')
    // ('p__x','q')：productId 含 __ → BAD_SPEC
    expect((await post('runAssembly', SUPER, { assemblyId: 'x2', productId: 'p__x', spec: 'q', sets: 1 })).error).toBe('BAD_SPEC')
    // ('p','_q')：合法组合（唯一定界不撞键）——过 BAD_SPEC 校验，因无此产品差异位而止步于 NO_PROFILE
    expect((await post('runAssembly', SUPER, { assemblyId: 'x3', productId: 'p', spec: '_q', sets: 1 })).error).toBe('NO_PROFILE')
    // 正常键照常：既有 seedBom 的 p1（本 describe 其余用例已验证 productId='p1' 全程可正常组装，不复述）
  })
})
