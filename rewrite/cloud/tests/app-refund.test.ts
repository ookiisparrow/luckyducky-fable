...
describe('refundCallback（黄金：防伪·核验 fail-closed·幂等·回补规则）', () => {
  ...
  it('大白话：退款成功后——未发货单回补库存；已发货单不回补（实物已出库，回补=幻影超卖）', async () => {
    ...
    // 对账痕 key 用 lineId（非裸 productId）——见下方专项用例：同 productId 不同 spec 不互相覆盖
    const o1 = control.dump('orders')[0]
    expect(o1['refunded.p1__基础款'] ?? o1.refunded?.['p1__基础款']).toBeTruthy() // 订单留对账痕
  ...
  // P1 修复（根因#14 失败必可观测）：退款回调晚到、此时售后单已被人工 rejectRefund 改成 rejected 时，
  // 旧版条件只认 applied/approved 两种情况、匹配不到任何行，函数静默 return——钱可能已经真退了但本地
  // 记录认为已拒绝，没有任何留痕信号。现须补一条高危告警落 anomalies 账本，供人工核对退款是否已实际
  // 到账、是否需要补偿处理；已是 refunded 的幂等重放不算异常，不告警。
  it('大白话：单号/金额都核验通过的成功回调，命中本地已是 rejected 的售后单——不翻状态但留高危告警；已是 refunded 的幂等重放不告警', async () => {
    control.setOpenId('')
    seedOrder()
    seedAS({ status: 'rejected', rejectedAt: 1500, rejectReason: '不符合退货规则' })
    await cb(SUCCESS)
    expect(control.dump('afterSales')[0].status).toBe('rejected') // 绝不误翻状态
    const anomalies = control.dump('anomalies')
    expect(anomalies.some((a: any) => a.code === 'SUCCESS_BUT_LOCAL_STATUS_MISMATCH')).toBe(true)

    control.reset()
    control.setOpenId('')
    seedOrder()
    seedAS({ status: 'refunded', refundedAt: 1500, refundTransactionId: 'wx-old' })
    await cb(SUCCESS)
    expect(control.dump('afterSales')[0].refundTransactionId).toBe('wx-old') // 幂等重放不改写
    expect(control.dump('anomalies').some((a: any) => a.code === 'SUCCESS_BUT_LOCAL_STATUS_MISMATCH')).toBe(false)
  })

  it('大白话：同一 productId 不同 spec 两条订单行各自退款成功——对账痕互不覆盖（P2：key 曾裸用 productId 会撞车）', async () => {
    control.setOpenId('')
    control.seed('orders', [
      {
        _id: 'o5',
    ...
    seedAS({ _id: 'o5__p1__红', orderId: 'o5', lineId: 'p1__红', productId: 'p1', spec: '红', refundAmount: 208 })
    seedAS({ _id: 'o5__p1__蓝', orderId: 'o5', lineId: 'p1__蓝', productId: 'p1', spec: '蓝', refundAmount: 188 })
    await cb({ out_refund_no: 'o5__p1__红', out_trade_no: 'o5', refund_status: 'SUCCESS', transaction_id: 'wx-r', amount: { refund: 20800 } })
    await cb({ out_refund_no: 'o5__p1__蓝', out_trade_no: 'o5', refund_status: 'SUCCESS', transaction_id: 'wx-b', amount: { refund: 18800 } })
    const o5 = control.dump('orders').find((o: any) => o._id === 'o5')
    const refunded = o5.refunded || {}
    expect(refunded['p1__红'] ?? o5['refunded.p1__红']).toBe(208) // 第一条退款痕未被第二条覆盖
    expect(refunded['p1__蓝'] ?? o5['refunded.p1__蓝']).toBe(188)
  })

  it('大白话：lineId 含点号（规格如 3.5mm）——对账痕键洗成下划线，不被 TCB 当嵌套路径拆层（合流保留深审 P3 修复）', async () => {
    control.setOpenId('')
    control.seed('orders', [
      {
        _id: 'o6',
        ...
        goods: 208,
        address: {},
        items: [{ productId: 'p2', lineId: 'p2__3.5mm', spec: '3.5mm', name: '钩针', price: 208, qty: 1, refundable: true }],
      },
    ])
    seedAS({ _id: 'o6__p2__3.5mm', orderId: 'o6', lineId: 'p2__3.5mm', productId: 'p2', spec: '3.5mm', refundAmount: 208 })
    await cb({ out_refund_no: 'o6__p2__3.5mm', out_trade_no: 'o6', refund_status: 'SUCCESS', transaction_id: 'wx-d', amount: { refund: 20800 } })
    const o6 = control.dump('orders').find((o: any) => o._id === 'o6')
    expect((o6.refunded || {})['p2__3_5mm'] ?? o6['refunded.p2__3_5mm']).toBe(208) // 点号已洗成 '_'，扁平一键
  })
