...
import { msHuman, mapReport, mapPanels, matchLabel, normalizeKb, mapCsat, mapCsatEntries, mapMessages, nextNodeId, duplicateNodeIds, mapQcRow, mapQcRows } from '../src/lib/mapCs'
...
describe('满意度明细行（批 B6·mapCsatEntries）', () => {
  it('大白话：无备注显 —；bad=差评 ≤2 星（供「查会话」按钮/行高亮判据，同 Csat.vue badOnly maxScore=2）；非数组回空', () => {
    ...
    expect(rows[1].bad).toBe(false) // 3 分不算差评
    expect(rows[2].bad).toBe(true) // 1-2 分算差评
...
describe('质检抽检行映射（批 B7·sessionKey≠externalUserId·聚合失败诚实省略非造 0）', () => {
  it('大白话：未评行 scored=false/score=0/note 空；已评行读 qc 字段；聚合字段带则透传、无则 messageCount=null 且首响回 —', () => {
    const pending = mapQcRow({ sessionKey: 'wxkf:kf1:eu123456789', externalUserId: 'eu123456789', qcSampledAt: 1783046400000, messageCount: 3, avgResponseMs: 61_000 })!
    expect(pending.sessionKey).toBe('wxkf:kf1:eu123456789') // sessionKey 是 csSession._id·非 externalUserId
    expect(pending.externalUserId).toBe('eu123456789')
    expect(pending.tailId).toBe('…456789') // 尾号脱敏
    expect(pending.scored).toBe(false)
    expect(pending.score).toBe(0)
    expect(pending.note).toBe('')
    expect(pending.messageCount).toBe(3)
    expect(pending.avgResponseLabel).toBe('1 分 1 秒')

    const noAgg = mapQcRow({ sessionKey: 'wxkf:kf1:eu2', externalUserId: 'eu2', qcSampledAt: 1 })!
    expect(noAgg.messageCount).toBeNull() // 聚合失败诚实省略·非造 0
    expect(noAgg.avgResponseLabel).toBe('—')

    const scored = mapQcRow({ sessionKey: 'wxkf:kf1:eu3', externalUserId: 'eu3', qcSampledAt: 1, qc: { score: 4, note: '不错', by: 'admin', at: 2 } })!
    expect(scored.scored).toBe(true)
    expect(scored.score).toBe(4)
    expect(scored.note).toBe('不错')
  })

  it('大白话：短标识不假掩；缺 sessionKey 的行被剔除；非数组回空', () => {
    expect(mapQcRow({ sessionKey: 'wxkf:kf1:eu1', externalUserId: 'ab' })!.tailId).toBe('ab')
    expect(mapQcRow({ externalUserId: 'eu1' })).toBeNull() // 无 sessionKey
    const rows = mapQcRows([{ sessionKey: 's1', externalUserId: 'e1' }, { externalUserId: 'no-key' }, null])
    expect(rows.map((r) => r.sessionKey)).toEqual(['s1']) // 无 key 的行被过滤，不留半档
    expect(mapQcRows(null)).toEqual([])
  })
})
