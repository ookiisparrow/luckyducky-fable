...
describe('首页 hero 映射（黄金 §九：缺档回退默认文案·不空屏不半空）', () => {
  it('大白话：没配置首页内容 → 完整默认文案；配了一半 → 缺的那半回退默认、配的那半用配置', () => {
    ...
    expect(empty.img).toBe('/static/hero-full.jpg') // 缺 CMS 图 → 品牌小鸭大图兜底（hero 门面不留灰占位）
    // CMS 配了图则用 CMS 图（默认可覆盖）
    expect(mapHero({ hero: { img: 'cloud://x/hero.png' } }).img).toBe('cloud://x/hero.png')
