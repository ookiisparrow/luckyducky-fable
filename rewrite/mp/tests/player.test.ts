...
import { flattenSegments, navSegment, createPlaybackCache, formatClock, clampSeek, lessonStrip, nearestMark, playbackModeFor, rotateSwapPlan, castReclaimDue, CAST_RECLAIM_MS } from '../lib/player'
...
describe('castReclaimDue（投屏回本机兜底判定·批7 纯函数）', () => {
  // 背景（批7 平台查证）：投屏断开/结束时平台事件不可靠（castinginterrupt 播完不触发），唯一兜底信号＝
  // 本机 video 重新开播（投屏中本机不播）。但投屏建立瞬间本机可能补发 play/pause 事件抖动——须有时间窗：
  // connected 置位后满 CAST_RECLAIM_MS 才认 play 事件为「播放已回手机」。
  it('大白话：非投屏态（空串/connecting）永不回收——本机播放本来就是常态', () => {
    expect(castReclaimDue('', 0, 99999)).toBe(false)
    expect(castReclaimDue('connecting', 1000, 99999)).toBe(false)
  })
  it('大白话：connected 但时间窗未满（投屏建立瞬间的事件抖动）不回收', () => {
    expect(castReclaimDue('connected', 10000, 10000 + CAST_RECLAIM_MS - 1)).toBe(false)
  })
  it('大白话：connected 且满窗——本机重新开播＝投屏已不在，回收', () => {
    expect(castReclaimDue('connected', 10000, 10000 + CAST_RECLAIM_MS)).toBe(true)
  })
  it('大白话：connected 但无置位时间戳（异常路径缺记录）保守不回收——宁可留投屏 UI 也不误踢正常投屏', () => {
    expect(castReclaimDue('connected', 0, 99999)).toBe(false)
  })
})
