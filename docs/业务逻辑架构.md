...
> 配套：目标设计（已归档）见 `archive/设计规格-课程电商系统.md`、需求见 `需求清单.md`；当前完成度见 `现状与路线.md`；安全/逻辑缺口见 `archive/审核报告合卷-202606.md`（定向审核节）+ `调试日志.md`。
...
> **DB 集合 + `events`（动态）的契约**（存什么 / 写入者 / 读取者 / 敏感点·集合数见 `系统事实.md` §9 DB Collection 正册）——**客观事实单一源、不在此手抄**（防漂移·连数都不抄）。
> 速览：users · products(+Draft) · courses(+Draft) · orders · afterSales · qrcodes · activations · reviews · events · progress · content · cards · adminConfig · config · uploadChunks。
...
- **⚠️ 已知缺口（链2）**：**只有上架、没有下架/删除**。`publishProduct` 写入 `products` 后该商品**永久可售**——`deleteDraft` 只删草稿、橱窗 featured=false 只是不上首页轮播、`getProducts` 仍全量下发。持续上新的店要停售某款时无路径（误发布也撤不回）。见 `待办与债.md` #12。
...
- **步骤**：详情选 SKU → 购物车（按 **id+规格 双键**，同款不同规格算两条）→ 结算 → `createOrder`（`PAY_MODE=mock` 直接 `paid`；`=real` 写 `pending`）→ real 时 `pay` 发起云调用支付拉起 `wx.requestPayment` → 微信回调 `payCallback` 置 `paid` → 支付成功页 → 订单详情**同一笔**。
- **闸门**：openid 必须有；条目契约（id 非空、数量正整数）；**价格一律按云端 `products` 现算**（前端传的价被忽略）；带规格则必须在云端 SKU 命中（否则 UNKNOWN_SKU 整单拒）；地址只收白名单字段；`pay` 只收**本人 pending 单**、金额取库内 `amount`；`payCallback` 幂等（重复通知只生效一次）。
- **云函数**：createOrder → pay → payCallback；closeExpiredOrders（定时关单）；getMyOrders / 订单详情读同一笔。
- **占位说明**：优惠券 ¥20 + 运费 0 为**开发期占位**（无条件抵扣每一单，无券归属/资格）——用户 2026-06-11 确认是占位、非缺陷；P4 做真实券系统时替换。
- **支付开关**：`config` 集合 `pay` 文档 `{ mode, subMchId }`，缺省 mock（行为=旧模拟支付，零回归）；商户号到位后切 real。
...
### 链 10 · 售后退款（申请 → 审核 → 退款 → 回写）—— P4 Batch 2（2026-06-12）
- **触发**：顾客在售后页对某条目申请退款（订单详情「申请退款」带单进入）。
- **步骤**：`applyRefund`（建 afterSales 单，金额云端分摊）→ 商家线下收寄回包装 + 验收扫码（链6 兜底）→ 控制台「售后退款」点同意 → `adminApi.approveRefund` 触发**退款工作流** → 微信原路退款 → refundnotify 工作流 → `refundCallback` 幂等置 `refunded` + 订单留痕。拒绝走 `rejectRefund`（必填原因）。
- **闸门**：openid 本人 + 订单 paid/shipped/done + 商品在单内 + 条目 `refundable !== false`（确认进课即失，链6）+ **一单一品一售后**（_id=orderId__productId 库级唯一）+ 同单累计退款 ≤ 实付 + 退款金额一律云端算（不信任前端与控制台传值）。
- **云函数**：applyRefund / getMyAfterSales → adminApi.listRefunds / approveRefund / rejectRefund → refundCallback。
...
```mermaid
stateDiagram-v2
  [*] --> pending: createOrder（PAY_MODE=real）
  [*] --> paid: createOrder（PAY_MODE=mock · 开发期）
  pending --> paid: payCallback（支付回调 · 幂等）
  pending --> closed: closeExpiredOrders / pay 惰性判（15 分钟超时 · 回补预留库存）
  closed --> paid: payCallback（关单后回调到达 · 重抢库存成功才复活）
  closed --> refund_required: payCallback（关单回补后晚到回调 · 库存已售罄 · 钱已收待人工退款）
  ...
  refund_required --> [*]
```
- **守的规则**：不能跳级（paid 不能直接 done）、不能倒退、不能重复确认。`pay` 只收 pending；`payCallback` 幂等（paid/shipped/done 上的重复通知静默确认）；`shipOrder` 只收 paid/shipped；`confirmReceive` 只收 shipped。
- **关单回补后晚到回调防超卖（审核 P0·根因#1/#2）**：超时关单会 `restoreStock` 回补预留库存；故 `closed → paid` 复活前 `payCallback` 须**重新 CAS 抢回 reserved 库存**才转 paid（抢不到=已被别人买走→进 `refund_required` 死信态：钱已收、无法履约、`[LD_ALERT] PAID_BUT_OOS` 告警人工退款）。杜绝「关单回补 + 晚到回调」双单争一份库存的超卖。`refund_required` 为终态，退款由商家凭 transactionId 人工发起（admin 退款执行工具列后续·待办#5b）。
...
> 这些闸门已有自动化测试守护（`tests/cloud/`·每个闸有反向用例·测试数随批增见 `重构日志` 最新批·别手抄），删一个闸对应用例会变红。
