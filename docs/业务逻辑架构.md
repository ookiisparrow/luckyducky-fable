...
> 配套：目标设计（已归档）见 `archive/设计规格-课程电商系统.md`、需求见 `需求清单.md`；当前完成度见 `项目现状.md`；安全/逻辑缺口见 `archive/审核报告-定向-202606.md` + `调试日志.md`。
...
## 二、数据集合地图（云数据库 16 个集合；另 `events` 由 trackEvent 动态写）
...
| `afterSales` | 售后退款单（一单一品一售后，金额/状态对账） | applyRefund / adminApi.approveRefund / refundCallback |
...
| `config` | 支付开关（PAY_MODE / flowId / refundFlowId） | 运维配置 |
