...
1. **读约定**：`CLAUDE.md`（全部工程约定·按「痛→不变量→守卫」组织）。
...
- **验收的共同语言 = `npm run check` 全绿**（structure+typecheck+lint+test）。改完必跑；红灯就是你的任务清单，报错是处方级的、照着修。
- **快速反馈**：改完一个文件先跑 `node scripts/check-structure.mjs`（约 1.4 秒，覆盖全部仓级不变量），别等到最后才发现登记漏了。
- **不许碰**：`scripts/check-*.mjs` 与 `.claude/`（验收机器本身，裁判不能被球员改写）、部署命令（`tcb`/`deploy-fns` 是人工授权动作）。
