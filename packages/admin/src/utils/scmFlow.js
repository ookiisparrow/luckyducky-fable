import { Package, LayoutTemplate, Calculator, ShoppingCart, Scissors, PackageOpen } from 'lucide-vue-next'

// 进销存 SCM 页面顺序单源（用户 2026-07-02「按工作流程顺序排序」拍板）：建档(物料/配方) → 算缺口(备货)
// → 补料(采购/外协) → 产出(打包)。Sidebar 导航组 与 顶部流程标签（components/ScmFlowTabs.vue）共用本表，
// 别在两处分别改顺序（防漂移）——新增/调整 SCM 页面只改这一处。
