...
import { COLLECTIONS, applyStockMoves, produceStock, listMaterialDocs } from '../../../../kit'
import { resolveBom } from '@luckyducky/shared'
...
export async function runAssembly({ db, data, agentId }: Ctx) {
  ...
  await applyStockMoves([{ materialId: `fg:${productId}__${spec}`, delta: sets }], { docType: 'assembly_in', docId: assemblyId, operator })
