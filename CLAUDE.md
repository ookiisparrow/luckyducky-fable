...
- **参考**：`系统事实.md`（客观事实总源：接口/集合/数据/守卫，机器维护）· `业务逻辑架构.md`（业务链心智地图）· `云开发参考.md`（云开发踩坑速查·通用 API 查官方）· `微信客服配置手册.md`（微信客服智能客服配置流程 + 排障 runbook）· `验收手册.md`（验收方法论 + 黄金路径）。需求见 `需求清单.md`；设计规格×2 目标设计长文归 archive。
