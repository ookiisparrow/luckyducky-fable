...
> - **活代码在 `rewrite/`**：微信端原生 TS+glass-easel（全 WebView 首发）、后端云开发 17 函数（`rewrite/cloud`）、admin/agent（Vue3+Vite）、Astro 内容站（`rewrite/site`）。本仓 `packages/` 为旧线字节级冻结参照（守卫 `oldline-frozen`·处置留后续拍板）；**旧线专用工程约定（原 §2/§5 正文与 §4「多端样式」⚙ 条目）已迁 `docs/旧线工程约定.md`**（2026-07-23 车队地基批3·参照旧线时再读），新线约定散在 `rewrite/*/README.md` 与各批记账——改动前先确认约定对哪条线生效，不要凭旧文字办事。
...
>
> 以下为工程约定：§2/§5/§4「多端样式」的旧线（`packages/` 冻结参照）条款原文已迁 `docs/旧线工程约定.md`（各节留指针·机器扫描面不变），其余治理/git/质量条目对全仓有效。
...
活线（`rewrite/`）：mp＝原生 TS+glass-easel（微信开发者工具本地编译，无 bundler 构建）· cloud/shared＝TS（esbuild 打包）· admin/agent＝Vue3+Vite+TS · site＝Astro。各包专项约定见 `rewrite/README.md` 与各包 README。**不要擅自更换技术栈，优先沿用现有模式。** 旧线（`packages/` 冻结参照）uni-app 技术栈与 dev/build 命令原文 → `docs/旧线工程约定.md`。
...
- **守卫自身可信（病根16·盲区体检 2026-07-18）**：守卫的**逻辑**不漂移，但守卫与现实的**绑定**（扫描面/指针/清单镜像）会漂——空样本＝绿不发信号。扫描面统一排除（dot 目录恒不入）+金丝雀夹具常驻；注册表 id 唯一；fileRule 活线活性下限（守冻结参照须显式白名单）；会动环境的工具脚本禁旧线引用。`[机器守: guard-id-unique]` `[机器守: guard-scan-liveness]` `[机器守: rw-toolchain-no-oldline]`

**多端样式（旧线专用 ⚙ 守卫族·原文已迁）**
（作用面＝旧线 `packages/miniapp`——conventions 扫描器只扫旧线；9 条 ⚙ 规则与条件编译纪律原文 → `docs/旧线工程约定.md`（守卫照跑不受迁移影响）。新线 `rewrite/mp` 为原生小程序、样式约定已随线重议（如 rpx 回归为平台正解），见 `rewrite/mp/README.md` 与其 rw-mp 守卫族。）
...
**重写期布局（ADR §23·M0 骨架）**：新代码只进 `rewrite/`（按里程碑立包 `rewrite/{golden,mp,shared,cloud,admin,agent,site}`，不预建空目录，约定见 `rewrite/README.md`）；`packages/` 旧线字节级冻结为参照基线 `[机器守: oldline-frozen]`（止血走 next 仓；有意识同步跑 `node scripts/freeze-oldline.mjs`）。**模块归属单源＝仓根 `modules.json`**（action/页面/集合 → 业务模块 + 危险分级 `[机器守: module-registry-complete]`）。旧线 miniapp 目录结构与命名约定原文 → `docs/旧线工程约定.md`（参照旧线时再读）。
