...
- `docs/业务逻辑架构.md`：三层架构总览 + 数据集合地图 + 9 条主要业务逻辑链 + 状态机 + 安全闸门一览（先读这份建立全局心智）。
- `docs/上线前占位清单.md`：开发期占位逐条决策追踪（做/不做/暂留/待讨论），上线收口照此清单。
...
- 代码按技术分层组织（不按业务链重组）；逻辑链「整体视图」看 `docs/业务逻辑架构.md`（决策 §17，受云函数独立部署等硬约束）。
