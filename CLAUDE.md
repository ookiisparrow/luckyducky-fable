...
- ✅ **购物车页**（`pages/cart`）：由 `store/cart.js`（Pinia）真实驱动 —— 空车态/有货态、条目选择、数量增减、全选、选中合计、浮动「去结算」栏。详情页/购物车推荐位「加入购物车」真正入车（按 id 合并 +1）。数据 `data/cart.js`（为你推荐）。「去结算」暂 Toast 占位（结算页是下一步）。强调色用 `$purple`。
...
- ✅ **结算页 + 支付成功页**（`pages/checkout`、`pages/paysuccess`）：地址(样例) + 店铺/商品(可改数量) + 搭配购买(勾选+数量) + 配送/优惠/积分/备注(展示) + 金额明细(实时联动) + 固定提交坞；提交→支付成功页(成功标+实付+订单号+两个出口)。
  ...
  - ⚠️ **占位**：地址用样例(地址簿编辑/选择待做)、配送/优惠券/积分/备注为展示+Toast、「查看订单」Toast、**无真实微信支付**(提交即视为成功)。
- ✅ **「我」个人中心主页**（`pages/me`，学习中枢版）：紫色资料头(头像/名/手机/简介/编辑) + 继续学习视频卡(进度) + 我的订单九宫格(待支付/待发货/待收货/已完成/退款，带角标) + 客服/地址 列表。数据 `data/profile.js`。
  ...
  - ⚠️ **占位（子流程后续做）**：编辑资料、订单各状态页、全部订单、客服、地址管理 均 Toast；继续学习进度为样例(无真实观看记录)。
  - 注：原占位页样式 `styles/placeholder.scss` 现已无人引用（cart/me 都已重写），可择机删除。
...
6. 个人中心：主页 ✅ 已完成；子流程待做（订单状态/待付款/地址簿/售后/评价/资料编辑）
