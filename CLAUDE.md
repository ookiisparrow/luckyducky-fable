...
- **边界尺子**：页面只编排；纯展示组件只 props/emits；跨页状态进 store、页面临时态留页；timer 必清理、storage key 用常量；storage/query/API 输入一律不可信；禁 `eval`/`v-html`；敏感信息不进日志。审核分级：P0 崩溃/数据错乱/交易错误/安全 ｜ P1 主流程错/状态不一致 ｜ P2 维护成本/重复 ｜ P3 风格/命名。
