// 订单域声明→生成（P3「安全处生成」spike）行为锁。
// 单源 packages/shared/src/order.spec.ts → 生成 order.ts（类型/常量/流转表）+ order-domain.generated.json。
// 本测试锁：① 生成器幂等（committed 态 --check 无漂移）；② 生成的 JSON 流转表自洽
// （每条 from/to ∈ 声明状态集）；③ 派生 TS 与 JSON 同源（状态集一致）。
// 反向自检：改 order.spec.ts 不重生成 → ① 红；篡改生成物 → ① 红。
...
describe('gen-order-domain（订单域声明→派生物·P3 安全处生成）', () => {
  ...
  it('生成的 TS 与 JSON 同源（状态联合 = JSON 状态集）', () => {
    const ts = readFileSync(ORDER_TS, 'utf8')
    // orders → OrderStatus；afterSales → AfterSaleStatus
    const typeName = { orders: 'OrderStatus', afterSales: 'AfterSaleStatus' }
    for (const coll of Object.keys(spec)) {
      const tn = typeName[coll]
      if (!tn) continue
      const m = ts.match(new RegExp(`export type ${tn} = ([^\\n]+)`))
      expect(m, `${tn} 未在 order.ts 生成`).toBeTruthy()
      const tsStates = new Set([...m[1].matchAll(/'([a-z]+)'/g)].map((x) => x[1]))
      expect([...tsStates].sort(), `${tn} 状态联合 ≠ JSON 状态集`).toEqual([...spec[coll].states].sort())
    }
  })

  it('orders/afterSales 状态机声明非空（spike 范围内两机存在）', () => {
    expect(spec.orders, 'orders 状态机缺失').toBeTruthy()
    expect(spec.afterSales, 'afterSales 状态机缺失').toBeTruthy()
    expect(spec.orders.transitions.length).toBeGreaterThan(0)
    expect(spec.afterSales.transitions.length).toBeGreaterThan(0)
  })
})
